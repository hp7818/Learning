...
//     <Box sx={{ width: "100%" }}>
//       <Box sx={{ mb: 2.5 }}>
//         <Typography variant="h5" fontWeight="600" sx={{ mb: 0.5, textAlign: "center" }}>🎉 Welcome, {currentUsername}!</Typography>
//         <Typography color="text.secondary" variant="body2"sx={{ textAlign: "center"}}>Here is System overview and analytics dashboard.</Typography>
//       </Box>
  ...
  return (
    <Box sx={{ width: "100%", px: { xs: 2, md: 4 }, py: 3, boxSizing: "border-box" }}>
      ...
      <Box sx={{ mb: 4, textAlign: "center" }}>
        <Typography variant="h4" fontWeight="600" sx={{ mb: 1 }}>🎉 Welcome, {currentUsername}!</Typography>
        <Typography color="text.secondary" variant="body1">Here is your system overview and analytics dashboard.</Typography>
      </Box>

      {/* Main Full-Width Grid Split */}
      <Grid container spacing={3} sx={{ width: "100%", margin: 0 }}>
        
        {/* LEFT PART: Stats Cards, Graph, and Pie Chart */}
        <Grid item xs={12} lg={7} sx={{ pl: "0px !important" }}>
          {/* Statistics Cards Grid */}
          <Grid container spacing={2} sx={{ mb: 3 }}>
            <Grid item xs={12} sm={6}><StatCard title="Total Users" value={stats.totalUsers} icon={<PeopleIcon />} color="primary" /></Grid>
            <Grid item xs={12} sm={6}><StatCard title="Total Roles" value={stats.totalRoles} icon={<SecurityIcon />} color="secondary" /></Grid>
            <Grid item xs={12} sm={6}><StatCard title="Categories" value={stats.totalCategories} icon={<CategoryIcon />} color="success" /></Grid>
            <Grid item xs={12} sm={6}><StatCard title="Documents" value={stats.totalDocuments} icon={<InsertDriveFileIcon />} color="warning" /></Grid>
          </Grid>
          ...
          <Card variant="outlined">
            <CardContent sx={{ p: 2 }}>
              <Typography variant="subtitle2" fontWeight="bold" sx={{ mb: 1 }}>User Role Distribution</Typography>
        ...
        {/* RIGHT PART: Recent Document Activity */}
        <Grid item xs={12} lg={5}>
          <Card variant="outlined">
            <CardContent sx={{ p: 2, "&:last-child": { pb: 2 } }}>
              <Typography variant="subtitle2" fontWeight="bold" sx={{ mb: 2 }}>Recent Document Activity</Typography>
              
              {/* Reduced Height Table with Scrollbar */}
              <TableContainer sx={{ maxHeight: 380, overflowY: "auto", width: 900 }}>
                <Table size="small" stickyHeader>
                  <TableHead>
                    <TableRow>
                      <TableCell sx={{ fontWeight: "bold", fontSize: "0.75rem", bgcolor: "action.hover" }}>Document Name</TableCell>
                      <TableCell sx={{ fontWeight: "bold", fontSize: "0.75rem", bgcolor: "action.hover" }}>Uploaded By</TableCell>
                      <TableCell sx={{ fontWeight: "bold", fontSize: "0.75rem", bgcolor: "action.hover" }}>Date</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {recentActivity.length > 0 ? (
                      recentActivity.map((activity, index) => (
                        <TableRow key={index} hover>
                          <TableCell>
                            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                              <DescriptionIcon fontSize="small" color="action" />
                              <Typography variant="body2" noWrap sx={{ maxWidth: 500 }}>{activity.name}</Typography>
                            </Box>
                          </TableCell>
                          <TableCell><Typography variant="body2" noWrap sx={{ maxWidth: 100 }}>{activity.uploadedBy}</Typography></TableCell>
                          <TableCell><Typography variant="body2" color="text.secondary" whiteSpace="nowrap">{activity.date}</Typography></TableCell>
                        </TableRow>
                      ))
                    ) : (
                      <TableRow>
